...
- **Bloqueio operacional pendente**: `lib/migrations/009-ingress-ranking-months-subscribed.sql` foi criada mas **ainda não aplicada em produção** — precisa da mesma autorização explícita do Luiz que toda migração anterior (003, 005, 006) recebeu antes de rodar no Neon SQL Editor. Sem ela, `months_subscribed` sempre grava/lê `NULL` (comportamento seguro, não quebra nada, só a seção de assinatura mostra "sem dados suficientes" até a migração rodar).
- **Autorizações concedidas pelo Luiz nesta feature**: nenhuma ainda — a migração 009 está pendente de autorização (ver acima).
- **UAT visual**: ainda não feito — o Luiz não revisou a aba na tela.
- **Next step**: (1) autorizar e aplicar a migração 009 em produção; (2) UAT visual da aba; (3) decidir sobre push/PR (branch só local); (4) opcionalmente, uma rodada de Tasks pra P7-P9.
