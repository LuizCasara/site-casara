...
- **Bloqueio operacional resolvido (12/09)**: com autorização explícita do Luiz, `casara.ingress_rankings` foi criada em produção (script pontual não-versionado, não `lib/schema.sql` automatizado) e o smoke test real do `POST/GET /api/ingress-rankings` passou: upsert, debounce (mesmo codinome, dados diferentes, negado), guarda do FencherLC (nunca escreve) — tudo confirmado contra o banco real, linha de teste apagada depois. `T4` em `tasks.md` marcado `[x]` por completo agora.
