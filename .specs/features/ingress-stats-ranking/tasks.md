...
- [x] Smoke manual contra produção (12/09/2026, com autorização explícita do Luiz): DDL de T1 aplicada em produção (rodada via script pontual não-versionado, `casara.ingress_rankings` confirmada com as 9 colunas + 3 índices). `POST` com `zzz-teste-descartar` → `written:true`, `overallScore:22.71`, `tier:"Bronze"`; `GET` devolveu a linha completa; segundo `POST` do mesmo codinome (dados diferentes) → `written:false`, dados preservados (debounce confirmado); `POST` com `codename:"FencherLC"` → `written:false`, `totalAgents` continua 1 (guarda do FencherLC confirmada, nunca escreve). Linha de teste apagada depois (`DELETE ... WHERE codename_key = 'zzz-teste-descartar'`) — tabela real ficou vazia e limpa.
...
**Done when**:
- [x] Todo texto (incluindo `CATS`/`SOON`) troca com o toggle
- [x] `npm run build` e `npm run lint` verdes
...
**Done when**:
- [x] Todo o `COPY` troca com o toggle
- [x] `npm run build` e `npm run lint` verdes
...
**Done when**:
- [x] As 3 mensagens trocam com o toggle
- [x] `npm run build` e `npm run lint` verdes
