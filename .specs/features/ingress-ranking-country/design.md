...
| `ALTER TABLE` em `casara.ingress_rankings`, tabela de produção já em uso real | `lib/migrations/002-ingress-ranking-country.sql` (novo) | Qualquer erro na migração afeta o ranking ao vivo | Coluna nullable, sem `DEFAULT`, sem reescrita de linhas existentes — operação de metadados rápida; aplicada só com autorização explícita antes de rodar em produção |
