...
| Precedência de pré-preenchimento do campo "Agente A"/"Agente B" ao abrir a aba Comparação | 1) parâmetros de URL válidos (se presentes) &gt; 2) par de agentes fechado pelos dois cliques do atalho de linha da tabela &gt; 3) `localStorage` ("meu agente", só preenche Agente A, só se ele estiver vazio) &gt; 4) vazio | Encadeamento lógico das três fontes de preenchimento automático já confirmadas separadamente em conversa; um link explícito sempre deve vencer uma conveniência implícita | n |
...
2. WHEN o visitante aciona "Comparar" numa linha e nenhum outro agente está marcado THEN o sistema SHALL apenas marcar aquele agente (linha destacada, controle em estado pressionado) e SHALL permanecer na aba "Ranking".
3. WHEN o visitante aciona "Comparar" na linha de um segundo agente, diferente do marcado THEN o sistema SHALL preencher "Agente A" com o agente marcado primeiro e "Agente B" com o da linha atual (sobrescrevendo qualquer valor anterior, inclusive o "meu agente" pré-preenchido), SHALL limpar a marcação e SHALL navegar para a aba "Comparação".
4. WHEN o visitante aciona "Comparar" na linha do agente já marcado THEN o sistema SHALL desfazer a marcação, sem trocar de aba.
5. The system SHALL manter esse controle utilizável por toque (tap) em telas estreitas, com alvo de toque de tamanho adequado (mínimo ~44×44px), consistente com o restante dos controles interativos da tabela.

**Independent Test**: na tabela de ranking, clicar "Comparar" numa linha → a linha fica destacada e a aba não muda; clicar "Comparar" em outra linha → aba Comparação abre com o 1º agente em A e o 2º em B; clicar duas vezes na mesma linha → marcação desfeita.
