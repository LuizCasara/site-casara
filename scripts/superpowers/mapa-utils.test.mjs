...
test('carrega as 311 linhas de dados do TSV', () => {
...
test('a distribuição real por fase bate com a seção 11 do spec (1067→311 rastreados, após excluir 7 arquivos de convenção especial que trocam de URL em route group — ver seção 5.1)', () => {
  const rows = loadMapaRows();
  const counts = {};
  for (const r of rows) counts[phaseOf(r)] = (counts[phaseOf(r)] ?? 0) + 1;
  assert.deepEqual(counts, { 1: 33, 2: 34, 4: 103, 5: 101, 6: 16, 7: 24 });
});
