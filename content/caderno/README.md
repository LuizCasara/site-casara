...
- **Este `README.md` não vira página.** A rota lê só os arquivos que começam com
  dígito (ver `app/(livros)/api/caderno/route.ts`), o que também é o que garante que a
  ordem exista.
...
é um componente React (`lib/livros/segredos/marcador-pdf.tsx`), porque o que ela faz é gerar um
