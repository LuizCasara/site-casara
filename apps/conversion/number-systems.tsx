...
const NumberSystemsConverter = () => {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      <h2 className="text-2xl font-bold mb-6">Conversor de Sistemas Numéricos</h2>
        ...
        <div className="p-4 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200 rounded-md mb-6">
          ...
          {values.showAllConversions ? (
            <div className="space-y-2">
              {numberSystems.map((system) => (
                <div key={system.id} className={`p-2 ${system.id === values.fromSystem ? 'bg-green-200 dark:bg-green-800 rounded' : ''}`}>
              ...
              {numberSystems.map((system) => {
                if (system.id !== values.fromSystem) {
                  return (
                    <div key={system.id}>
                      <p>
                        <span className="font-medium">{numberSystems.find(sys => sys.id === values.fromSystem).name}:</span> {results.originalValue}
                      </p>
                      <p>
                        <span className="font-medium">{system.name}:</span> <span className="font-mono">{results.conversions[system.id]}</span>
                      </p>
                    </div>
                  );
                }
