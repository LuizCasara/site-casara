...
test('computeSeasonalEngagement: agente sem a chave em extra_stats não conta no sum nem no reportedCount daquela chave', () => {
    ...
    assert.deepEqual(result.firstSaturdayEvents, {sum: 3, reportedCount: 1})
    assert.deepEqual(result.secondSundayEvents, {sum: 7, reportedCount: 2})
})

test('computeSeasonalEngagement: valor não-numérico é tratado como ausente, nunca coagido ou lançado', () => {
    const rows = [makeRow({extra_stats: {apolloTokens: 'muitos'}}), makeRow({extra_stats: {apolloTokens: 10}})]
    const result = computeSeasonalEngagement(rows)
    assert.deepEqual(result.apolloTokens, {sum: 10, reportedCount: 1})
})
...
test('computeSeasonalEngagement: extra_stats null (agente nunca mandou) não quebra e não conta em nenhuma chave', () => {
    ...
    for (const key of SEASONAL_ENGAGEMENT_KEYS) {
        assert.deepEqual(result[key], {sum: 0, reportedCount: 0})
    }
})
...
test('computeSubscription: ninguém com months_subscribed preenchido -> hasData false, sem 0%/média zerada', () => {
    ...
    assert.deepEqual(result, {hasData: false, percentSubscribed: null, avgMonthsAmongSubscribed: null})
})
...
test('computeNerdStats: formato do retorno tem as 6 chaves do design (totals, byFaction, averages, hallOfFame, seasonalEngagement, subscription)', () => {
    const result = computeNerdStats([makeRow()], 1)
    assert.deepEqual(
        Object.keys(result).sort(),
        ['averages', 'byFaction', 'hallOfFame', 'seasonalEngagement', 'subscription', 'totals']
    )
})
