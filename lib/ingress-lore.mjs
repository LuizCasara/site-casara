 ...
 * @param {ReturnType<typeof loadLore>} [lore]
 * @param {'pt'|'en'} [lang] — escolhe `blurb`/`facts` (pt, default) ou
 *   `blurbEn`/`factsEn` (en) da mesma entrada. Parâmetro novo no final da
 *   assinatura para não quebrar quem já chama com 4 argumentos.
 * @returns {{blurb: string, facts: {n: number, label: string}[]} | null}
 */
export function medalLore(slug, value, daysPlaying, lore = loadLore(), lang = 'pt') {
    const entry = lore[slug]
    if (!entry) return null
    const v = Number.isFinite(value) ? value : 0
    const days = Math.max(1, daysPlaying)
    const sourceFacts = (lang === 'en' ? entry.factsEn : entry.facts) || []
    ...
    const blurb = lang === 'en' ? entry.blurbEn : entry.blurb
    return {blurb, facts}
}
