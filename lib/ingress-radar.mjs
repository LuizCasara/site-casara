...
export const RADAR_AXES = [
    {
        ...
        label: 'Construção',
        parts: [
            {key: 'resonatorsDeployed', label: 'Resonadores implantados', ref: 200000, badge: 'builder'},
    ...
    },
    {
        id: 'destruicao',
        label: 'Destruição',
        parts: [{key: 'resonatorsDestroyed', label: 'Resonadores destruídos', ref: 300000, badge: 'purifier'}],
    },
    {
        id: 'exploracao',
        label: 'Exploração',
 ...
 *   parts:{key:string,label:string,value:number,ref:number,ratio:number}[]}[]}
...
export function computeRadarAxes(stats) {
    ...
    return RADAR_AXES.map((axis) => {
        const parts = axis.parts.map((p) => {
            ...
            return {key: p.key, label: p.label, value, ref: p.ref, ratio: value / p.ref}
        })
