 ...
 * mudar a cada render. Limitado a DESK_MAX_BOOKS porque o spec prevê 1 a 3
 * livros "lendo agora" simultâneos; um quarto livro simplesmente não aparece,
 * sem erro.
 */
...
/**
 * Distância vertical entre uma prancha e a de cima. Altura do livro (0,30m)
 * mais folga pra ele não raspar na prateleira acima e pra caber o gesto de
 * hover, que puxa o livro pra fora e o inclina.
 */
export const SHELF_ROW_SPACING_M = 0.42;

export const SHELF_ROWS = 2;

/**
 * Divide os livros em `rows` fileiras de largura parecida.
 *
 * Balanceia por LARGURA ACUMULADA, não por quantidade: as espessuras variam
 * quase 4x entre o livro mais fino e o mais grosso (30mm a 110mm), então
 * partir a lista no meio pela contagem daria duas fileiras de comprimentos
 * bem diferentes — e a estante ficaria visivelmente torta.
 *
 * A ordem de entrada é preservada dentro de cada fileira, e as fileiras saem
 * na ordem de leitura (a primeira é a de cima). Quem chama depende disso pra
 * casar cada livro com sua lombada no atlas.
 */
export function splitShelfRows(shelfBooks, rows = SHELF_ROWS) {
    if (rows <= 1 || shelfBooks.length === 0) return [shelfBooks];

    const larguraAlvo = shelfWidthM(shelfBooks) / rows;
    const fileiras = Array.from({length: rows}, () => []);

    let fileiraAtual = 0;
    let larguraAtual = 0;
    for (let i = 0; i < shelfBooks.length; i++) {
        const livro = shelfBooks[i];
        const restantes = shelfBooks.length - i;
        const fileirasRestantes = rows - fileiraAtual;

        // Passa pra próxima fileira quando esta já encheu — mas nunca se isso
        // deixaria alguma fileira seguinte vazia. Sem essa segunda condição,
        // um acervo com poucos livros muito grossos jogaria todos na primeira
        // fileira e desenharia uma prateleira vazia embaixo.
        const cheia = larguraAtual >= larguraAlvo;
        // `>=`, não `>`: depois de avançar, as fileiras que ainda precisam de
        // ao menos um livro são `fileirasRestantes - 1`. Ter exatamente essa
        // quantidade de livros sobrando já basta — exigir um a mais fazia o
        // último livro grudar na fileira anterior e a de baixo nascer vazia.
        const sobraLivroPraTodas = restantes >= fileirasRestantes - 1;
        if (cheia && fileiraAtual < rows - 1 && sobraLivroPraTodas) {
            fileiraAtual++;
            larguraAtual = 0;
        }

        fileiras[fileiraAtual].push(livro);
        larguraAtual += livro.thicknessM + SHELF_GAP_M;
    }
