 ...
 * testável com node --test (mesmo espírito de lib/livros/sala/livros-routing.mjs).
 *
 ...
 * `id` casa com o tipo `Viewpoint` de components/livros/sala/CameraRig.tsx.
 */
 ...
 * de recados (que então estava em x≈0,68) e a bíblia (x≈2,22) — um metro e meio
 * de distância. O resultado é que nada ficava bem enquadrado, e a tela do monitor
 * em especial ficava pequena demais para ler o que estava tocando.
 *
 * **Eram cinco e são quatro.** O quadro de recados abria a lista até 27/08/2026,
 * quando mudou para a parede lateral esquerda para não ser engolido pela segunda
 * estante do acervo (ver `QUADRO_RECOMENDACOES` em Room.tsx) e virou décor
 * clicável, sem parada. O que sobrou são só os objetos da MESA, o que é mais
 * coerente do que um canto de trabalho que começava do outro lado da parede.
 */
export const FOCOS_DO_PC = [
    /*
      A gaveta abre a lista porque a ordem é a posição no mundo: o trilho varre o
      canto da esquerda para a direita, e ela está em x ≈ 1,01, à esquerda dos
      monitores (1,84). Enfileirar por ordem de chegada faria a câmera saltar para
      a frente e voltar.
      ...
      **`foraDoTrilho` a tira da NAVEGAÇÃO, não da sala.** A gaveta é a única
      ...
      Ela ser a PRIMEIRA e estar fora do trilho ao mesmo tempo é o caso que
      `subParadasDaCena` já tratava: a lista de índices navegáveis do canto passa
      a ser [1, 2, 3], e não [0, 1, 2].
    */
 ...
 *
 * `[sala, camping, mesa, estante, 2026, 2025, …, PC, monitores, alto-falante,
 * bíblia]` — as sub-paradas entram logo depois da cena
 ...
 * pontas — ao contrário de folhear livros (`vizinhosDe`, em lib/livros/sala/livros-shelf.mjs),
 ...
 * partir das datas de leitura (ver lib/livros/sala/shelf-years.mjs).
 */
