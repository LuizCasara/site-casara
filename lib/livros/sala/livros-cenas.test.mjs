...
test('sair da gaveta cai no vizinho dela no trilho, não no começo da sala', () => {
    ...
    //
    // Para trás a chegada é o PLANO ABERTO do canto, `{pc, null}`, e não uma
    // sub-parada: a gaveta é a primeira da lista desde que o quadro de recados
    // mudou de parede, então o vizinho anterior dela no trilho é a própria cena.
    const naGaveta = {cena: 'pc', sub: GAVETA};
    assert.deepEqual(paradaVizinha(naGaveta, 1, 3), {cena: 'pc', sub: GAVETA + 1});
    assert.deepEqual(paradaVizinha(naGaveta, -1, 3), {cena: 'pc', sub: null});
});
...
test('subVizinha pula um buraco no MEIO da lista de índices', () => {
    // Lista sintética, e não a do canto do PC: hoje a única parada oculta é a
    // gaveta, e ela é a PRIMEIRA — o buraco cai na ponta, não no meio. Este
    // teste guarda o caso geral (uma parada oculta cercada dos dois lados),
    // que é o que faria andar de um em um cair justamente nela. Sem uma lista
    // própria, o dia em que a lista real mudar de forma leva a garantia junto.
    const COM_BURACO = [0, 2, 3];
    assert.equal(subVizinha(0, 1, COM_BURACO), 2);
    assert.equal(subVizinha(2, -1, COM_BURACO), 0);
    assert.equal(subVizinha(1, 1, COM_BURACO), 2, 'saindo da oculta para cima');
    assert.equal(subVizinha(1, -1, COM_BURACO), 0, 'saindo da oculta para baixo');
});

test('no canto do PC a parada oculta é a PRIMEIRA, e descer dela sai do zoom', () => {
    // A gaveta abre FOCOS_DO_PC desde que o quadro de recados mudou de parede,
    // então os índices navegáveis começam em 1 e nada existe à esquerda dela.
    // Descer daí não tem vizinho para onde ir e cai na regra da base: `null`,
    // que é a cena inteira em quadro.
    const subs = subParadasDaCena('pc', 0);
    assert.equal(GAVETA, 0, 'a gaveta deixou de ser a primeira — reveja este teste');
    assert.equal(subVizinha(GAVETA, 1, subs), GAVETA + 1);
    assert.equal(subVizinha(GAVETA, -1, subs), null);
    assert.equal(subVizinha(subs[0], -1, subs), null, 'a primeira navegável também sai do zoom');
