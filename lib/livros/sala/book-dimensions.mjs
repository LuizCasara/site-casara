 ...
 *
 * .mjs de propósito, mesma razão de lib/livros/acervo/book-utils.mjs e lib/livros/sala/contraste.mjs: é o
 * único jeito de rodar `node --test` neste projeto (`lib/**\/*.test.mjs`). A
 * geração da textura em si (lib/livros/sala/spine-canvas.ts) usa `<canvas>` do DOM e não
...
/**
 * Converte livros crus do banco (formato de `lib/livros/acervo/books.ts`) no formato que
 * `components/livros/sala/Book.tsx` espera, com as dimensões já calculadas.
...
// por alguém. O valor é o mesmo da tabela de offsets à mão que existia aqui
// antes (ver `layoutDeskBooks`), para a pilha não mudar de amplitude.
export const DESK_OFFSET_MAX_M = 0.018;
 ...
 * mudar a cada render.
 *
 * **Teve um teto de 3 e não tem mais.** O desalinho de cada volume vinha de uma
 * tabela de três offsets escrita à mão, e um quarto livro "lendo" simplesmente
 * não aparecia — sem erro, sem aviso, sem lugar nenhum na sala. Aconteceu de
 * verdade em 28/08/2026, com um livro recém-cadastrado invisível. Agora os
 * offsets são DERIVADOS DO SLUG e a pilha não tem tamanho máximo, que é
 * exatamente o caminho que `layoutTorreDeLivros` já tinha tomado logo abaixo,
 * pela mesma razão: com três dá para escrever os offsets à mão, com uma fila
 * que cresce, não.
 *
 * Medido antes de tirar o teto: quatro livros dão 23,6cm de pilha, com o topo a
 * 0,496m do chão — abaixo do braço da poltrona, a 0,55m — e ocupam até 18cm do
 * centro do tampo de 78cm, contra a xícara em 0,27 e os óculos em -0,25. Quem
 * ler seis de uma vez terá uma pilha de uns 35cm, e é ele que decide se isso é
 * demais para uma mesa de centro.
 */
export function layoutDeskBooks(books) {
    let alturaAcumulada = 0;
    return books.map((livro) => {
        const tx = hashString01(`${livro.slug}:desk-x`);
        const tz = hashString01(`${livro.slug}:desk-z`);
        // A chave da rotação é a MESMA de antes de propósito: assim os livros
        // que já estavam na mesa continuam tortos exatamente como estavam.
        const tr = hashString01(`${livro.slug}:desk-rotation`);
        const y = alturaAcumulada + livro.thicknessM / 2;
        alturaAcumulada += livro.thicknessM;
        return {
            slug: livro.slug,
            x: (tx * 2 - 1) * DESK_OFFSET_MAX_M,
            y,
            z: (tz * 2 - 1) * DESK_OFFSET_MAX_M,
            rotationY: (tr * 2 - 1) * DESK_ROTATION_VARIANCE_RAD,
        };
    });
}
 ...
 * deitados de `layoutDeskBooks`, e como ela sem teto de quantidade — uma fila de
 * leitura pode ter dez títulos, e todos precisam estar lá para poderem ser
 * clicados.
 *
 * O desalinho de cada volume é DERIVADO DO SLUG, e foi aqui que essa mecânica
 * nasceu: com três livros dá para escrever os três offsets à mão, com uma fila
 * que cresce, não. A pilha da mesa era a tabela à mão, e passou a usar isto no
 * dia em que um quarto "lendo" sumiu da sala. Determinístico pelo mesmo motivo
 * de sempre — a torre não pode mudar de forma a cada render.
 *
 ...
 * decidir se dois anos cabem no mesmo nicho (lib/livros/sala/shelf-years.mjs).
 */
