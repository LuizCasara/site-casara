...
import {corDeTextoSobre} from '../sala/contraste.mjs';
import {isbn13Para10} from './book-utils.mjs';
...
/**
 * URL da capa na Amazon a partir do ISBN-13. Para livros, o ASIN da Amazon é o
 * ISBN-10, e este caminho legado (/images/P/<asin>.01.…) serve a capa direto,
 * sem raspar a página do produto nem precisar de chave de API.
 *
 * É a segunda fonte de capa do acervo, não a primeira: a Open Library continua
 * vindo antes no cadastro. Esta existe porque a Open Library falha justamente
 * nas edições brasileiras — dos 12 livros importados em 07/08/2026, 6 não
 * estavam lá e 5 desses 6 tinham capa aqui.
 *
 * Quando a Amazon NÃO tem a imagem, ela responde 200 com um GIF transparente
 * de 43 bytes em vez de 404 — o mesmo truque da Open Library, e a razão de
 * `BYTES_MINIMOS` existir. Quem chama não precisa tratar isso: `baixarCapa` já
 * rejeita a resposta pelo tamanho e cai no placeholder.
 */
export function capaDaAmazon(isbn) {
    const isbn10 = isbn13Para10(isbn);
