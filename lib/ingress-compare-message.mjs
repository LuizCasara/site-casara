...
function selfEvolution(a, b, when) {
    ...
    const grew = afAxes.filter((ax, i) => ax.onyxRatio > beAxes[i].onyxRatio).length
    const gap = timeGap(before.capturedAt, after.capturedAt)

    const evoLines = afAxes.map((ax, i) => {
        const d = Math.round((ax.onyxRatio - beAxes[i].onyxRatio) * 100)
        const dtxt = d > 0 ? `+${d}` : d < 0 ? `${d}` : '0'
        return `${padR(clip(ax.label, 16), 16)} ${padL(pct(beAxes[i].onyxRatio), 6)} ${padL(pct(ax.onyxRatio), 6)} ${padL(dtxt, 4)}`
    })
    const evoTable = [
...
export function compareMessages({a, b, vsOwner = false, when = new Date()}) {
    ...
    const axisLines = aAxes.map((ax, i) => {
        ...
        return `${padR(clip(ax.label, 16), 16)} ${padL(pct(ax.onyxRatio), 6)} ${padL(pct(bAxes[i].onyxRatio), 6)}${mark}`
    })
    ...
    const caption = [
        ...
        `_${quem}_`,
        '',
        '*Padrão de jogo — % do nível Onyx* (`<` / `>` = líder)',
        '```',
        axisTable,
        '```',
