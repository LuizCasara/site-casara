...
test('computeBadge: atMax -> beyond aponta pra próxima dobra do Onyx', () => {
    const onyx = byKey('liberator').tiers.onyx
    const r = computeBadge(byKey('liberator'), onyx * 2.5)
    assert.equal(r.atMax, true)
    assert.equal(r.pct, null)
    assert.equal(r.next, null)
    assert.equal(r.beyond.multiple, 2)
    assert.equal(r.beyond.label, 'Onyx ×3')
    assert.equal(r.beyond.target, onyx * 3)
    assert.ok(Math.abs(r.beyond.pct - 0.5) < 1e-9)
    assert.ok(Math.abs(r.beyond.remaining - onyx * 0.5) < 1e-6)
})

test('computeBadge: exatamente no Onyx -> beyond é a 1ª dobra em 0%', () => {
    const onyx = byKey('liberator').tiers.onyx
    const r = computeBadge(byKey('liberator'), onyx)
    assert.equal(r.beyond.label, 'Onyx ×2')
    assert.equal(r.beyond.pct, 0)
})
