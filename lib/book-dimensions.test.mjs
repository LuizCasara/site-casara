...
test('layoutDeskBooks é determinístico e não perde nenhum livro', () => {
    ...
    // **Este teste já garantiu o oposto**: a pilha tinha teto de 3, e o quarto
    // livro "lendo" desaparecia da sala sem erro. Aconteceu de verdade em
    // 28/08/2026. O que se garante agora é que TODO livro recebe posição — é a
    // única forma de o sumiço silencioso não voltar.
    assert.equal(layout1.length, PILHA.length);
    for (const livro of PILHA) {
        assert.ok(layout1.some((s) => s.slug === livro.slug), `${livro.slug} ficou sem lugar`);
    }

    // E com uma pilha grande também — o teto não pode reaparecer como um número
    // maior escrito em outro lugar.
    const muitos = Array.from({length: 9}, (_, i) => ({slug: `x${i}`, thicknessM: 0.04}));
    assert.equal(layoutDeskBooks(muitos).length, 9);
});
