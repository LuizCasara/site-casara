...
function badgeFor(statKey, value) {
    ...
    if (!def) return null
    return {badgeSlug: def.slug, tier: computeBadge(def, value).tier}
}
 ...
 * principal, `compareRankingRows`). Cada stat do radar carrega a badge/tier
 * que o próprio recordista atingiu nesse stat (quando existe badge pra ele —
 * `portalsNeutralized` é sintético, sem badge própria), pro ícone da medalha
 * aparecer na cor certa em vez de um genérico.
 * @param {Record<string, unknown>[]} rows
 * @returns {{perStat:Record<string,{codenameKey:string,codename:string,faction:string,countryCode:string|null,value:number,badgeSlug:string|null,tier:string|null}|null>, lifetimeAp:object|null, recursions:object|null}}
 */
export function computeHallOfFame(rows) {
    const perStat = {}
    for (const key of RADAR_STAT_KEYS) {
        const record = pickRecordCoerceZero(rows, (r) => r.stat_values?.[key])
        const badge = record ? badgeFor(key, record.value) : null
        perStat[key] = record ? {...record, badgeSlug: badge?.badgeSlug ?? null, tier: badge?.tier ?? null} : null
    }
...
/** As 9 chaves de `extra_stats` que compõem "engajamento em eventos sazonais" (NERD-24). */
 ...
 * months_subscribed IS NOT NULL` — nunca sobre o total geral.
 * @param {Record<string, unknown>[]} rows
 * @returns {{hasData:boolean, percentSubscribed:number|null, avgMonthsAmongSubscribed:number|null}}
 */
export function computeSubscription(rows) {
    ...
    if (withData.length === 0) {
        return {hasData: false, percentSubscribed: null, avgMonthsAmongSubscribed: null}
    }
    ...
    return {hasData: true, percentSubscribed, avgMonthsAmongSubscribed}
}

/**
 * Orquestrador — chamado por `app/ingress/ranking/page.tsx` (SSR) com TODAS
