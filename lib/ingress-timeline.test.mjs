...
import {collectAcquisitions, annotateLaneGaps, formatGap, groupLanes} from './ingress-timeline.mjs'
...
test('groupLanes: uma raia por medalha, tiers em ordem, maior tier, raias por 1ª data', () => {
    const rows = annotateLaneGaps(
        collectAcquisitions(
            {
                medalDates: {
                    trekker: {gold: '2021-08-01', onyx: '2024-03-12'},
    ...
    assert.deepEqual(
        lanes.map((l) => l.slug),
        ['recursion', 'trekker', 'liberator'],
    )
    const trekker = lanes.find((l) => l.slug === 'trekker')
    assert.equal(trekker.latestTier, 'onyx')
    assert.equal(trekker.firstDate, '2021-08-01')
    assert.deepEqual(
        trekker.tiers.map((t) => t.tier),
        ['gold', 'onyx'],
    )
    assert.equal(trekker.tiers[1].prevTier, 'gold')
})

test('groupLanes: lista vazia/nula -> []', () => {
    assert.deepEqual(groupLanes([]), [])
    assert.deepEqual(groupLanes(null), [])
})
