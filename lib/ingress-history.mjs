...
export function projectNextTier(history, badgeDef, currentValue) {
    ...
    const ms = Date.now() + daysToGo * 86_400_000
    // Taxa minúscula × limiar gigante (ex.: MU do Illuminator) estoura o Date
    // máximo e `toISOString()` lança RangeError — trata como "sem previsão".
    if (!Number.isFinite(ms) || ms > 8.64e15) return {reason: 'sem-progresso'}
