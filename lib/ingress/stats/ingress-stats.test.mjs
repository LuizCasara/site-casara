...
// docs/ingress/ingress-perfil-fencherlc.md.
const HEADER =
...
test('parseAppExport aborta se o arquivo não é um export (nenhuma coluna reconhecida)', () => {
    assert.throws(
        () => parseAppExport('lat\tlng\tname\n-24.9\t-53.4\tCatedral\n'),
        /Agent Name/,
    );
});

test('parseAppExport lê o export mesmo quando o colar troca os tabs por espaço', () => {
    const spaced = REAL_EXPORT.replace(/\t/g, ' ');
    const tabbed = parseAppExport(REAL_EXPORT);
    const loose = parseAppExport(spaced);
    assert.equal(loose.agent.codename, tabbed.agent.codename);
    assert.equal(loose.agent.faction, tabbed.agent.faction);
    assert.equal(loose.agent.level, tabbed.agent.level);
    assert.deepEqual(loose.stats, tabbed.stats);
    assert.equal(loose.capturedAt, tabbed.capturedAt);
})

test('parseAppExport (espaço): coluna desconhecida multi-palavra não desalinha o resto', () => {
    // "Orion Tokens" não está no STAT_COLUMNS; entre colunas conhecidas
    const header = 'Time Span Agent Name Agent Faction Date (yyyy-mm-dd) Time (hh:mm:ss) Level Hacks Orion Tokens Distance Walked'
    const row = 'ALL TIME zezextreme12cr Enlightened 2026-09-08 22:19:57 9 1043 50 607'
    const r = parseAppExport(`${header}\n${row}`)
    assert.equal(r.agent.codename, 'zezextreme12cr')
    assert.equal(r.stats.hacks, 1043)
    assert.equal(r.stats.distanceWalkedKm, 607)
    assert.ok(r.warnings.some((w) => /Orion Tokens/.test(w)))
})
...
test('STAT_COLUMNS: toda entrada usada em UI (kind !== "meta") tem labelEn não-vazio', () => {
    const uiColumns = STAT_COLUMNS.filter((c) => c.kind !== 'meta');
    assert.ok(uiColumns.length > 0);
    for (const c of uiColumns) {
        assert.equal(typeof c.labelEn, 'string', `${c.key} sem labelEn`);
        assert.ok(c.labelEn.trim().length > 0, `${c.key} com labelEn vazio`);
    }
});

test('STAT_COLUMNS: entradas kind === "meta" não precisam de labelEn (só o CLI usa)', () => {
    const metaColumns = STAT_COLUMNS.filter((c) => c.kind === 'meta');
    assert.equal(metaColumns.length, 3);
});

test('STAT_GROUPS: todo grupo tem titleEn não-vazio', () => {
    for (const g of STAT_GROUPS) {
        assert.equal(typeof g.titleEn, 'string', `${g.id} sem titleEn`);
        assert.ok(g.titleEn.trim().length > 0, `${g.id} com titleEn vazio`);
    }
});
