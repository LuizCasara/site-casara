 ...
 * por sua vez lê `badge-catalog.json` via `lib/ingress/catalog/ingress-catalog.mjs`, `fs`-based).
 *
 * Os limiares vêm do catálogo de badges (fonte da verdade) e a posição de tier
 * de cada stat vem de `lib/ingress/stats/ingress-tier-position.mjs`: linear até o Onyx, e
 * 5 + log₂(valor/Onyx) depois dele — cada dobra do Onyx soma 1 posição. Este
 * módulo agrega as posições pelos 5 eixos de `lib/ingress/stats/ingress-radar.mjs` e tira a
 ...
 * Onyx; além do Onyx, 5 + log₂(valor/Onyx)). Ver `lib/ingress/stats/ingress-tier-position.mjs`.
 ...
 * bilíngue de `lib/ingress/catalog/ingress-tiers.mjs`; default `'pt'` preserva o comportamento
