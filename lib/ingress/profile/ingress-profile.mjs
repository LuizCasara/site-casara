 ...
 * `data/ingress/fencherlc.json` e alimenta a página `/ingress/fencherlc`.
 *
 * `buildProfile` parte da saída de `parseAppExport` (`lib/ingress/stats/ingress-stats.mjs`).
 * `mergeGdprDump` incorpora as séries temporais e listas de portais do dump
 * GDPR (lidas pelo CLI em `scripts/ingress/ingress.mjs`). Ambos são puros e
 * idempotentes — sem `Date.now()`, ordem de chave estável.
 */
import {appendSnapshot} from './ingress-history.mjs'
...
export function buildProfile(parsedExport, opts = {}) {
    ...
    const orderedStats = sortedStats(stats);
    ...
    return {
        ...
        stats: orderedStats,
        // Um ponto por export com capturedAt novo. Perfil antigo sem history é
        // migrado (nasce com o snapshot atual).
        history: appendSnapshot(previous?.history, {t: capturedAt, stats: orderedStats}),
        medalDates: previous?.medalDates ?? {},
        eventBadges: previous?.eventBadges ?? [],
...
export function mergeGdprDump(profile, dumpData) {
    ...
    // Snapshots históricos do dump entram no history, inclusive antes do primeiro.
    let history = Array.isArray(profile.history) ? profile.history : [];
    for (const snap of d.snapshots || []) history = appendSnapshot(history, snap);
