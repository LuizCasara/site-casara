...
/**
 * Polígonos normalizados (0..1, x=lng/y=lat) numa janela quadrada em torno de
 * um centro — o que os heróis decorativos (`HeroMesh`, em `/ingress/fencherlc`
 * e `/ingress/ranking`) desenham. Extraído aqui pra não duplicar a normalização
 * lat/lng -> viewBox entre os dois heróis.
 * @param {{lat:number, lng:number}} center
 * @param {{span?:number, level?:number, cap?:number}} [opts]
 * @returns {[number,number][][]}
 */
export function heroMeshPolygons(center, opts = {}) {
    const span = opts.span ?? 0.06;
    ...
    const latToY = (lat) => (center.lat + span - lat) / (2 * span);
    const lngToX = (lng) => (lng - (center.lng - span)) / (2 * span);
    return cells.map((c) => c.ring.map(([lat, lng]) => [lngToX(lng), latToY(lat)]));
}
