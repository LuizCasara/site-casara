 ...
 * (`lib/ingress/catalog/ingress-catalog.mjs` → `data/ingress/badge-catalog.json`, gerado do
 * ingress.plus, com os limiares que o scanner do jogo mostra diferente
 * sobrescritos — ver `TIER_OVERRIDES` em `scripts/ingress/ingress-catalog-gen.mjs`).
 * O cálculo de tier é feito no render, não congelado no perfil.
 * `statKey` referencia uma chave de `lib/ingress/stats/ingress-stats.mjs`.
 */
import {coreBadges} from './ingress-catalog.mjs'

// As constantes de tier vivem em `ingress-tiers.mjs` (sem I/O) e são
// re-exportadas aqui por compatibilidade com quem já importava daqui.
export {TIERS, TIER_RANK, TIER_LABELS, TIER_COLOR} from './ingress-tiers.mjs'
import {TIERS} from './ingress-tiers.mjs'
 ...
 * @returns {{key,name,statKey,value,tier,atMax,pct,next:{tier:string,remaining:number}|null,beyond:{multiple:number,label:string,target:number,remaining:number,pct:number}|null}}
 *   `pct` = progresso 0..1 do tier atual até o próximo (`null` se atMax).
 *   `beyond` = progresso além do Onyx: a próxima "dobra" (Onyx ×2, ×3…), só
 *   preenchido quando `atMax`.
 */
export function computeBadge(badgeDef, statValue) {
    ...
    let beyond = null
    if (!atMax) {
        const nextTier = tier === 'none' ? TIERS[0] : TIERS[TIERS.indexOf(tier) + 1]
        ...
        const onyx = badgeDef.tiers.onyx
        const multiple = Math.max(1, Math.floor(value / onyx))
        const target = (multiple + 1) * onyx
        beyond = {
            multiple,
            label: `Onyx ×${multiple + 1}`,
            target,
            remaining: target - value,
            pct: Math.max(0, Math.min(1, (value - multiple * onyx) / onyx)),
        }
    }
    return {
        key: badgeDef.key,
...
export function nextMedal(badges) {
    ...
    for (const b of badges) {
        // `!Number.isFinite` também cobre o NaN de um catálogo com tier faltando
        // ou dois tiers iguais — sem isso o badge quebrado vira `best` e trava.
        if (b.atMax || b.pct == null || !Number.isFinite(b.pct)) continue
        if (
            best === null ||
            b.pct > best.pct ||
