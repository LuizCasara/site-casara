...
/**
 * O que está pendurado na parede do fundo, à esquerda da estante.
 *
 * O Gorillaz é quadrado porque a arte é capa de disco — o formato do quadro
 * acompanha a imagem, não o contrário.
 *
 * **É um só, e já foram dois.** O quadro de recados morava em x = 0,68, entre a
 * estante e a prateleira aérea, e saiu daqui em 27/08/2026 — a segunda estante
 * do acervo passou de cenário futuro a próximo livro cadastrado, e ela avança
 * até x = 0,87, ou seja, por cima de 40 dos 44cm dele. Não havia para onde
 * escorregar: entre a borda da segunda estante e a prateleira aérea sobram
 * 8,5cm. Ele foi para a parede lateral esquerda, empilhado sob o pôster de
 * Hunter x Hunter, que é a única faixa da sala que nenhum móvel disputa — o
 * mesmo desfecho que o pôster já tinha tomado, e pelo mesmo motivo. Ver
 * `POSTER_HUNTER` e `QUADRO_RECOMENDACOES` em `Room.tsx`.
 *
 ...
 * mais apertado que a parede já teve era o quadro de recados contra a estante, a
 * 4,2cm, e um limiar de 5cm teria reprovado a sala como ela estava.
 *
 * Esse par não existe mais: com o quadro na parede lateral, o vizinho mais
 * próximo que sobra é o Gorillaz contra a estante, a 33,5cm com duas estantes. O
 * limiar continua em 3cm porque ele não descreve a sala de hoje e sim o ponto em
 * que duas peças param de ler como duas — mas quem o reapertar não tem mais uma
 * medida real segurando o número.
 */
